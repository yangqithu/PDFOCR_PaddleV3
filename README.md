...
基于 PaddleOCR 3 的 PDF OCR 桌面工具，为扫描版 PDF 添加可搜索、可复制的隐藏文字层，并通过版面分析减少页眉、页脚和页码对正文识别的干扰。v31 重点提高了处理速度、降低了显存占用，并加入双联页拆分功能。
...
> [!IMPORTANT]
> 当前发布的是 **v31 源代码版本**，主要面向横排文档。程序按 `gpu:0` 配置，需要 NVIDIA GPU 及匹配的 PaddlePaddle GPU 环境；模型和 CUDA 不包含在仓库中。**竖排文字写入仍未解决，不应将本项目用于依赖精确竖排文字层的任务。**
...
- **多档清晰度**：图形界面提供 150、220 和 300 DPI。
- **多语种识别**：界面内置中英文、繁体中文、日文、韩文、法文、德文等常用模型代码，也支持手动输入 PaddleOCR 语言代码。
- **改进图形界面**：高级选项区域可滚动，长说明会自动换行，并按使用频率重新排列选项。
...
- **当前按 GPU 环境设计**：代码将识别设备设为 `gpu:0`，没有受支持的 CPU 工作模式。若安装的是 CPU 版 Paddle，底层可能静默回退，速度会明显下降。
...
6. 检查高级选项；第一次使用建议保持默认。高级选项区域可用鼠标滚轮查看。
7. 点击“启动识别序列”，等待处理完成。
8. 默认会自动打开处理完成的文档。
...
> [!NOTE]
> 截图用于展示整体布局。v31 的高级选项区域已经改为可滚动，并增加了双联页拆分和分段计时选项。
...
指定 OCR 页码和主识别 DPI 目前仍是图形界面功能，没有对应的命令行参数。双联页的跳过页码与拆分 DPI 可以通过命令行设置。
...
这是当前版本的已知限制。v31 沿用了稳定的横排文字写入逻辑，没有重新启用会破坏正常横排识别的竖排实验，暂时没有可靠修复。`--split-rtl` 只改变双联页左右两半的输出顺序，并不能解决竖排 OCR。
...
当前仓库提供可审阅和自行运行的 **v31 源代码版本**，不包含 EXE、CUDA、PaddlePaddle GPU 安装包或模型文件。后续若制作包含模型或运行时的 Windows 安装包，需要另行处理体积、CUDA 兼容性、第三方许可证和不同硬件环境测试。
